...
To run the custom suite of unit test for JS Util use the following sequence of commands that will install dependencies
...
- Increment (look at `CHANGELOG.md` for semver changes) the `version` field in `package.json`.
...
JS Util is licensed under the [Apache License, Version 2.0](http://www.apache.org/licenses/).
