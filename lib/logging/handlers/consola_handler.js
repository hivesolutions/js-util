...
Logging.ConsolaHandler = function(instance, noDefaults) {
    ...
    // resolves the numeric debug level in a backwards compatible
    // way: consola 2 exposed a `LogLevel.Debug` enum that maps to
    // 4, while consola 3 dropped the enum but still accepts the
    // same numeric value, so falling back to 4 when the enum is
    // absent keeps the behaviour identical across both versions
    var debugLevel = consola.LogLevel ? consola.LogLevel.Debug : 4;
