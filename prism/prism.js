        ...
        tokenloop : for (var token in grammar) {
            ...
            for (var i = 0; i < strarr.length; i++) {
                ...
                if (match) {
                    ...
                    var from = match.index - 1 + lookbehindLength, match = match[0].slice(lookbehindLength), len = match.length, to = from
                            + len, before = str.slice(0, from + 1), after = str.slice(to
                            + 1);
...
if (Prism.languages.markup) {
    ...
    Prism.hooks.add("after-highlight", function(env) {
                ...
                for (var i = 0, t; t = env.tokenStack[i]; i++) {
                    env.highlightedCode = env.highlightedCode.replace("{{{PHP"
                                    + (i + 1) + "}}}", Prism.highlight(t,
                                    env.grammar, "php"));
                }
